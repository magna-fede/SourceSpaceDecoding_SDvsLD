...
with open("/imaging/hauk/users/fm02/first_output/evoked_responses/0804_evoked_flipped.P", 'rb') as f:
      evoked = pickle.load(f)
...
for roi in kkROI:
    ...
    plt.savefig(f'/imaging/hauk/users/fm02/final_dTtT/evoked/flip_avg/flipped  evoked_LDvsSD_{roi}.png', format='png')
    plt.show();
...
for roi in kkROI:
    ...
    by_label = dict(zip(labels, handles))
    plt.legend(by_label.values(), by_label.keys())
    plt.savefig(f'/imaging/hauk/users/fm02/final_dTtT/evoked/flip_avg/flipped evoked_SDvsSD_{roi}.png', format='png')
    # plt.legend();
    plt.show();
