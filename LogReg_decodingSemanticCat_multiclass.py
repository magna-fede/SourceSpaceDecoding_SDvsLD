...
for sub in np.arange(0, 18):
    ...
    for task in trials_semK.keys():
        print(f"Analysing participant {sub}, task {task}")
        # not working if more different number of trials in different 
        # categories
        # X_y = pd.melt(pd.DataFrame.from_dict(subt[task]))
        
        # this works
        X_y = pd.DataFrame.from_dict(subt[task], orient='index').transpose()
        X_y = pd.melt(X_y)
        X_y = X_y.dropna()
        
        X = np.stack(X_y['value'])
        y = X_y['variable']
...
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/0315_LogReg_AVG_SemK_scores_multiclass.P",
          'wb') as outfile:
    pickle.dump(df_to_export,outfile)

##########################################################
    
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/0315_LogReg_AVG_SemK_scores.P", 'rb') as f:
...
Mt_clust, Mclusters, Mp_values, MH0 = permutation_cluster_1samp_test(
    MLK_mean-.2, n_jobs=1, threshold=threshold, adjacency=None,
    n_permutations='all')
# Put the cluster data in a viewable format
...
Ft_clust, Fclusters, Fp_values, FH0 = permutation_cluster_1samp_test(
    FRT_mean-.2, n_jobs=1, threshold=threshold, adjacency=None,
    n_permutations='all')
# Put the cluster data in a viewable format
...
Ot_clust, Oclusters, Op_values, OH0 = permutation_cluster_1samp_test(
    ODR_mean-.2, n_jobs=1, threshold=threshold, adjacency=None,
    n_permutations='all')
# Put the cluster data in a viewable format
...
SDt_clust, SDclusters, SDp_values, SDH0 = permutation_cluster_1samp_test(
    SD_mean-.2, n_jobs=1, threshold=threshold, adjacency=None,
    n_permutations='all')
# Put the cluster data in a viewable format
