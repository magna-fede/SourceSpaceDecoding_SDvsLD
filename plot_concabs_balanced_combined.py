...
def reorg_patterns(sbj_list):
    ...
    try:
        ...
        for task in final.keys():
            final[task] = dict.fromkeys(['visual', 'hand', 'hear', 'abstract'])
        for task in reorg:
...
for i in range(0, 18):
    scores['SD'].append(np.stack([np.array(scores['milk'].loc[i]),
                              np.array(scores['fruit'].loc[i]),
                              np.array(scores['odour'].loc[i])]).mean(axis=0))
