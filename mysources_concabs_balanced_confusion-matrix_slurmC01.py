...
def individual_confusion_matrix(sub):
    ...
    clf = make_pipeline(StandardScaler(),  # z-score normalization
                        ...
                        LinearModel(LogisticRegression(C=0.0001,
                                                       solver='lbfgs',
                                                       max_iter=1000))) 
    ...
    #                             max_iter=1000)
    # clf2 = GridSearchCV(logreg,                    # model
    #                    param_grid = parameters,   # hyperparameters
    #                    scoring='accuracy',        # metric for scoring
    #                    cv=5)                     # number of folds
    # time_decod = SlidingEstimator(clf2)
    # X = []
    # y = []
    # for semK in kk2:
    ...
    # time_decod.fit(X, y)
    # C_value = []
    # for i in range(300): C_value.append(time_decod.estimators_[i].best_params_)
    # model_scores = []
    # for i in range(300): model_scores.append(time_decod.estimators_[i].best_score_)
    ...
    with open(f"/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/abs_balanced_scores_{sub}_C0001.P",
                  'wb') as outfile:
            pickle.dump(scores,outfile)            
    with open(f"/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/abs_balanced_confusion_matrix_{sub}_C0001.P",
              'wb') as outfile:
