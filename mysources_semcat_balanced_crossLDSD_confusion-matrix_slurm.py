...
def individual_confusion_matrix(sub):
    ...
    for roi in kkROI:
        ...
        y_predicted = time_decod.predict(X_ld)
        
        # scoring the LD trials (but trained on SD!).
        scores[roi] = time_decod.score(X_ld, y_ld)

        for i in range(0, 300):
            conf_mat[roi].append(confusion_matrix(y_ld, y_predicted[:,i],
                                                        labels=['visual',
                                                                'hand',
                                                                'hear',
                                                                'abstract'],
    ...
    with open(f"/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/crissLDSD_balanced_scores_{sub}.P",
                  'wb') as outfile:
            pickle.dump(scores,outfile)            
    with open(f"/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/crossLDSD_balanced_confusion_matrix_{sub}.P",
              'wb') as outfile:
