# -*- coding: utf-8 -*-
"""
Created on Thu Sep  9 15:50:15 2021
...
### of each ROI independently in detecting the task.
...
for sub in np.arange(0  ,18):
    ...
    for roi in kkROI:
        ...
        X_mlk = np.concatenate([np.stack(mlks[roi]),np.stack(lds[roi])])
        y_mlk = np.array(['milk']*np.stack(mlks[roi]).shape[0] + ['LD']*np.stack(lds[roi]).shape[0])
        
        X_frt = np.concatenate([np.stack(frts[roi]),np.stack(lds[roi])])
        y_frt = np.array(['fruit']*np.stack(frts[roi]).shape[0] + ['LD']*np.stack(lds[roi]).shape[0])
        
        X_odr = np.concatenate([np.stack(odrs[roi]),np.stack(lds[roi])])
        y_odr = np.array(['odour']*np.stack(odrs[roi]).shape[0] + ['LD']*np.stack(lds[roi]).shape[0])
        ...
        # 5 cross-validations
        scores_mlk[roi] = cross_val_multiscore(time_decod, X_mlk, y_mlk, cv=5)
        scores_frt[roi] = cross_val_multiscore(time_decod, X_frt, y_frt, cv=5)
        scores_odr[roi] = cross_val_multiscore(time_decod, X_odr, y_odr, cv=5)
    ...
    for roi in kkROI:
        ...
        avg_score[roi] = np.mean([scores_mlk[roi].mean(0),scores_frt[roi].mean(0),scores_odr[roi].mean(0)],0)
...
# when finished, save participants scores
# this will be retrieved in another script for visualization
df_to_export = pd.DataFrame(list_avg_scores)
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/0928_ROIs_avg_scores.P", 'wb') as outfile:
    pickle.dump(df_to_export,outfile)
df_to_export = pd.DataFrame(list_mlk_scores)
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/0928_ROIs_mlk_scores.P", 'wb') as outfile:
    pickle.dump(df_to_export,outfile)
df_to_export = pd.DataFrame(list_frt_scores)
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/0928_ROIs_frt_scores.P", 'wb') as outfile:
    pickle.dump(df_to_export,outfile)
df_to_export = pd.DataFrame(list_odr_scores)
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/0928_ROIs_odr_scores.P", 'wb') as outfile:
    pickle.dump(df_to_export,outfile)
