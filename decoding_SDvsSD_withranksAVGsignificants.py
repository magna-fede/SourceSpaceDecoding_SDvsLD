...
for sub in np.arange(1  ,18):
    ...
    mlkfrt_selected = pd.DataFrame(index=kkROI,
                                columns=range(300))
    frtodr_selected = pd.DataFrame(index=kkROI,
                                columns=range(300))
    odrmlk_selected = pd.DataFrame(index=kkROI,
                                columns=range(300))
    ...
    for i in range(300):
        coefscores_mf_ps = pd.DataFrame(zip(ROI_vertices,
                          SelectKBest(k='all').fit(X_mlkfrt[:,:,i],
                                                    y_mlkfrt).pvalues_))
        df = pd.DataFrame(zip(ROI_vertices, patterns_mlkfrt[:,i]))
        ...
        coefscores_fo_ps = pd.DataFrame(zip(ROI_vertices,
                          SelectKBest(k='all').fit(X_frtodr[:,:,i],
                                                    y_frtodr).pvalues_))
        df = pd.DataFrame(zip(ROI_vertices, patterns_frtodr[:,i]))
        ...
        coefscores_om_ps = pd.DataFrame(zip(ROI_vertices,
                          SelectKBest(k='all').fit(X_odrmlk[:,:,i],
                                                    y_odrmlk).pvalues_))
        df = pd.DataFrame(zip(ROI_vertices, patterns_odrmlk[:,i]))
    ...
    s = pd.DataFrame(np.vstack([mlkfrt_selected,
                                frtodr_selected,
                                odrmlk_selected]),
