...
for sub in np.arange(0, 18):
    ...
    for task in trials_avg3.keys():
        for roi in kkROI:
            ...
            time_gen.fit(X, y)
            score = time_gen.score(X, y)
            scores[task][roi].append(score)
...
#           'wb') as outfile:
with open("/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/scores_TimeGen.P", "wb") as outfile:
