...
for sub in np.arange(0  ,18):
    ...
    # BUT ONLY TRUE FOR LINEAR MODEL OBEJECT
    # AND LDA IS NOT, IS LINEAR DISCRIMINANT OBJECT
    ...
    # here I manually calculate a unique covariance matrix as X.T.dot(X) 
    # with shape n_vertices*n_vertices
    # this will be multiplied with each single coefficient patterns at each timepoint


    X_mlk_long = []
    for i in range(300):
        X_mlk_long.extend(X_mlk[:,:,i])
    
    X_mlk_long = np.array(X_mlk_long)
    cov_X_mlk_long = X_mlk_long.T.dot(X_mlk_long)
    
    patterns_mlk = []
    for i in range(300):
        patterns_mlk.append(cov_X_mlk_long.dot(coef_mlk[:,i]))
    ...
    X_frt_long = []
    for i in range(300):
        X_frt_long.extend(X_frt[:,:,i])
    
    X_frt_long = np.array(X_frt_long)
    cov_X_frt_long = X_frt_long.T.dot(X_frt_long)
    
    patterns_frt = []
    for i in range(300):
        patterns_frt.append(cov_X_frt_long.dot(coef_frt[:,i]))
    ...
    X_odr_long = []
    for i in range(300):
        X_odr_long.extend(X_odr[:,:,i])
    
    X_odr_long = np.array(X_odr_long)
    cov_X_odr_long = X_odr_long.T.dot(X_odr_long)
    
    patterns_odr = []
    for i in range(300):
        patterns_odr.append(cov_X_odr_long.dot(coef_odr[:,i]))

    patterns_odr = np.array(patterns_odr).T
    
    df2 = pd.DataFrame(zip(ROI_vertices, 
                           patterns_mlk,
                           patterns_frt,
                           patterns_odr),
                       columns=['ROI',
    ...
    for i in range(len(df2)):
        avg.append(np.mean([df2['milk'][i],
                            df2['fruit'][i],
                            df2['odour'][i]],0))
    df2['avg'] = avg
    
    SDLD2_coefficients.append(df2)
