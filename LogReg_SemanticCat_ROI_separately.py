    ...
    for task in trials_semK.keys():
        for roi in kkROI:
            for semKvsemK in comb:
                X = np.concatenate([trials_semK[task][semKvsemK[0]][roi],
                                        trials_semK[task][semKvsemK[1]][roi]])
                
                y = np.array([semKvsemK[0]]*len(trials_semK[task][semKvsemK[0]][roi]) + \
                                 [semKvsemK[1]]*len(trials_semK[task][semKvsemK[1]][roi]))
...
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/1128_LogReg_ROI_SemK_scores.P",
          'wb') as outfile:
    pickle.dump(df_to_export,outfile)

    
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/1128_LogReg_ROI_SemK_scores.P", 'rb') as f:
...
for participant in df_to_export['ld']:
    for roi in kkROI:
        avg_each_combination = np.array(participant[roi]).mean(axis=1)
...
for roi in kkROI:
    ...
    Lt_clust, Lclusters, Lp_values, H0 = permutation_cluster_1samp_test(
        LD_mean[roi]-.5, n_jobs=1, threshold=threshold, adjacency=None,
        n_permutations='all')
...
for roi in kkROI:
    ...
    Mt_clust, Mclusters, Mp_values, H0 = permutation_cluster_1samp_test(
        MLK_mean[roi]-.5, n_jobs=1, threshold=threshold, adjacency=None,
        n_permutations='all')
    # Put the cluster data in a viewable format
    p_clust = np.ones((1,300))
...
for roi in kkROI:
    ...
    Ft_clust, Fclusters, Fp_values, H0 = permutation_cluster_1samp_test(
        FRT_mean[roi]-.5, n_jobs=1, threshold=threshold, adjacency=None,
        n_permutations='all')
    # Put the cluster data in a viewable format
    p_clust = np.ones((1,300))
...
for roi in kkROI:
    ...
    Ot_clust, Oclusters, Op_values, H0 = permutation_cluster_1samp_test(
        ODR_mean[roi]-.5, n_jobs=1, threshold=threshold, adjacency=None,
        n_permutations='all')
    # Put the cluster data in a viewable format
    p_clust = np.ones((1,300))
