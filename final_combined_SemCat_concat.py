...
def run_patterns_semcat_concat(sub):
...
# First we will reorganise the data in pandas dataframe, instead of dict.
    ...
    for semK in trials_avg3['sd']:
        ...
        trials_avg3['sd'][semK] = np.concatenate([trials_avg3['mlk'][semK],
                                                  trials_avg3['frt'][semK],
                                                  trials_avg3['odr'][semK]])
    del(trials_avg3['mlk'])
    del(trials_avg3['frt'])       
    del(trials_avg3['odr'])
    ...
    for task in trials_avg3.keys():
        ...
        # because this is a multinomial classification, we will have 5 patterns
        # one for each category
        # when visually inspecting them, they look reasonably similar
        # or at least not that different, so just average them
        ### for i in range(0, 5): sns.lineplot(x= times, y=pd.DataFrame(pattern[:,i,:]),
        ###     index=ROI_vertices).groupby(ROI_vertices).apply(rms).loc['lATL'])
        ### make sure seaborn and rms are imported
        patterns[task] = pd.DataFrame(pattern.mean(axis=1), index=ROI_vertices)

    # save the scores ...

    with open(f"/imaging/hauk/users/fm02/final_dTtT/combined_ROIs/SemCat/patterns_concat_{sub}.P",
              'wb') as outfile:
        pickle.dump(patterns,outfile)
        
    # save the scores ...
    with open(f"/imaging/hauk/users/fm02/final_dTtT/combined_ROIs/SemCat/scores_concat_{sub}.P",
              'wb') as outfile:
