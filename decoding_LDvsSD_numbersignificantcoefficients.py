...
for sub in np.arange(0, 18):
    ...
    for i in range(300):
        coefscores_mlk_ps = pd.DataFrame(zip(ROI_vertices,
                          SelectKBest(k='all').fit(X_mlk[:,:,i],
                                                    y_mlk).pvalues_))
       
        df = pd.DataFrame(zip(ROI_vertices, patterns_mlk[:,i]))

        df_less05 = df.loc[coefscores_mlk_ps[1]<.05]
        ...
        mlk_selected[i] = v_counts['number_coef']   
        
        
        coefscores_frt_ps = pd.DataFrame(zip(ROI_vertices,
                          SelectKBest(k='all').fit(X_frt[:,:,i],
                                                    y_frt).pvalues_))
        df = pd.DataFrame(zip(ROI_vertices, patterns_frt[:,i]))

        df_less05 = df.loc[coefscores_frt_ps[1]<.05]
        ...
        frt_selected[i] = v_counts['number_coef']


        coefscores_odr_ps = pd.DataFrame(zip(ROI_vertices,
                          SelectKBest(k='all').fit(X_odr[:,:,i],
                                                    y_odr).pvalues_))
        df = pd.DataFrame(zip(ROI_vertices, patterns_odr[:,i]))

        df_less05 = df.loc[coefscores_odr_ps[1]<.05]
        ...
        odr_selected[i] = v_counts['number_coef'] 
    ...
    s = pd.DataFrame(np.vstack([mlk_selected,frt_selected,odr_selected]),
                     index=index,columns=range(300))
...
with open("//cbsu/data/Imaging/hauk/users/fm02/first_output/1111_LDvsSD_ncoef_less05.P", 'wb') as outfile:
