...
for roi in kkROI:
    ...
    for cluster in clusters[roi]:
        ...
        ax=sns.heatmap(get_clust,
                    annot=True,
                    xticklabels=np.unique([3,4,5,6,7,8,9]),
                    yticklabels=np.unique([3,4,5,6,7,8,9]),
                    cmap="viridis",)
        ax.set(xlabel="Predicted", ylabel="True")
        ax.set_title(f"{roi}, cluster {str(i)}")
        plt.tight_layout()
        plt.savefig(f"//imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/Figures/{roi}_{str(i)}_confusionmatrix_n_letters.png", format="png");
        plt.show()
