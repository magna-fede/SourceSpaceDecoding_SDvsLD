...
def individual_confusion_matrix(sub):
    ...
    conf_mat = dict.fromkeys(kkROI)
    for roi in conf_mat.keys():
        conf_mat[roi] = []
    ...
    for roi in kkROI:
        ...
        y_ld = []
        for semK in kk2:
            X.append(trials_avg3['SD'][semK][roi])
            X_ld.append(trials_avg3['LD'][semK][roi])
            y.extend([semK]*len(trials_avg3['SD'][semK][roi]))
            y_ld.extend([semK]*len(trials_avg3['LD'][semK][roi]))
        X = np.concatenate(X)
        X_ld = np.concatenate(X_ld)
        y = np.array(y)
        y_ld = np.array(y_ld)
        ...
        # Train using SD blocks (because there are more trials)
        time_decod.fit(X_ld, y_ld)
        
        # Predicted LD semantic category
        y_predicted = time_decod.predict(X)
        
        # scoring the LD trials (but trained on SD!).
        scores[roi] = time_decod.score(X, y)

        for i in range(0, 300):
            conf_mat[roi].append(confusion_matrix(y, y_predicted[:,i],
                                                        labels=['visual',
                                                                'hand',
                                                                'hear',
                                                                'abstract'],
    ...
    with open(f"/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/crossSDfromLD_balanced_scores_{sub}.P",
                  'wb') as outfile:
            pickle.dump(scores,outfile)            
    with open(f"/imaging/hauk/users/fm02/final_dTtT/individual_ROIs/SemCat/crossSDfromLD_balanced_confusion_matrix_{sub}.P",
              'wb') as outfile:
